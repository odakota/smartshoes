...
import com.odakota.tms.business.auth.repository.LoginFailedRepository;
...
public class LoginService {
     ...
     * @param resource {@link ResetPasswordResource}
     */
    public void changePassword(ResetPasswordResource resource) {
        User user = userRepository
                .findByIdAndDeletedFlagFalse(userSession.getUserId())
                .orElseThrow(() -> new CustomException(MessageCode.MSG_INVALID_USERNAME_PASS, HttpStatus.BAD_REQUEST));
        if (!passwordEncoder.matches(resource.getOldPassword(), user.getPassword())) {
     ...
     *
     * @param resource  LoginResource
     * @param loginType LoginType
     * @return User
     */
    private User validateUser(LoginResource resource, LoginType loginType) {
        User user = userRepository.findByUsernameOrPhoneAndDeletedFlagFalse(resource.getUsername(), resource.getPhone())
                                  .orElseThrow(() -> new CustomException(MessageCode.MSG_INVALID_USERNAME_PASS,
                                                                         HttpStatus.BAD_REQUEST));
        // check password
        if (loginType.equals(LoginType.ACCOUNT) &&
            !passwordEncoder.matches(resource.getPassword(), user.getPassword())) {
            LoginFailed loginFailed = new LoginFailed();
            loginFailed.setPhone(resource.getPhone());
            loginFailed.setUsername(resource.getUsername());
            loginFailed.setCustomer(false);
            loginFailedRepository.save(loginFailed);
            // check the total number of login failures
            if (loginFailedRepository.countByUsernameOrPhone(resource.getUsername(), resource.getPhone()) >= 5) {
                if (!user.isLockFlag()) {
                    user.setLockFlag(true);
                    userRepository.save(user);
                }
                throw new CustomException(MessageCode.MSG_USER_LOCKED, HttpStatus.CONFLICT);
            }
            throw new CustomException(MessageCode.MSG_INVALID_USERNAME_PASS, HttpStatus.BAD_REQUEST);
        }
        // check user's lock or unlock status
        if (user.isLockFlag()) {
            throw new CustomException(MessageCode.MSG_USER_LOCKED, HttpStatus.CONFLICT);
        }
        // delete loginFailed record if login success
        loginFailedRepository.deleteByUsernameOrPhone(resource.getUsername(), resource.getPhone(), false);
        return user;
    }
}
