...
public interface PermissionRepository extends BaseRepository<Permission, PermissionCondition> {
    ...
    @Query("select p from Permission p where exists (select pr from PermissionRole pr join Role r " +
           "on r.id = pr.roleId join UserRole ur on r.id = ur.roleId where p.id = pr.permissionId and ur.userId = ?1 " +
           "and pr.deletedFlag = false ) and p.deletedFlag = false order by p.id, p.sortNo asc ")
