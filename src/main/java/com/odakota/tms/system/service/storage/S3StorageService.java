...
public class S3StorageService {
    ...
    @Value("${storage-service.bucket-name}")
     ...
     */
    public List<StorageItem> list(final String path) {
     ...
     */
    public List<StorageItem> listFiles(final String path) {
        return this.list(path).stream()
                   .filter(tmp -> tmp.getType().equals(StorageItemType.FILE))
                   .collect(Collectors.toList());
    }

    public List<StorageItem> listDirectories(final String path) {
        return this.list(path).stream()
                   .filter(tmp -> tmp.getType().equals(StorageItemType.FOLDER))
                   .collect(Collectors.toList());
    }

    public Optional<StorageItem> getFile(final String path) {
        try {
     ...
     * @return file list
     */
    public List<StorageItem> find(final String path, final String pattern) {
     ...
     */
    public void get(final String path, final OutputStream output) {
        try {
            S3Object object = amazonS3.getObject(bucketName, path);
     ...
     */
    public void put(final String path, final InputStream input, final StorageItemMetadata metadata) {
    ...
    public void remove(final String path) {
        try {
            getS3Object(path)
                    .map(tmp -> {
                        amazonS3.deleteObject(bucketName, tmp.getKey());
                        return tmp;
                    })
    ...
    private Optional<S3Object> getS3Object(final String path) {
        // try to get as file
        try {
            S3Object object = amazonS3.getObject(bucketName, path);
